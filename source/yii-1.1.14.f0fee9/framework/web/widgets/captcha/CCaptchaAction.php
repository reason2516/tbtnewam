...
class CCaptchaAction extends CAction
{
	...
	protected function generateVerifyCode()
	{
		...
		$letters = '123456789012345678901';//bcdfghjklmnpqrstvwxyz
		$vowels = '12334';//aeiou
		$code = '';
		for($i = 0; $i < $length; ++$i)
		{
	...
	protected function getSessionKey()
	{
		//return self::SESSION_VAR_PREFIX . Yii::app()->getId() . '.' . $this->getController()->getUniqueId() . '.' . $this->getId();
                return self::SESSION_VAR_PREFIX . $this->getController()->getUniqueId() . '.' . $this->getId();//为了实现session同步却掉getId
	}
